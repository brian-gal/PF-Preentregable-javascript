...
// Esta función permite al usuario elegir la dificultad del juego
...
// Esta función genera un número aleatorio de cierta cantidad de cifras basado en la dificultad seleccionada
...
// Estas funciónes simplemente asignan un mensaje en base a una condicion
...
function asignaPalabraAcierto() { //corregimos la palabra para que quede bien el texto envase a los aciertos
    if (aciertos == 1) {
        return "acierto";
    } else {
        return "aciertos";
...
// Esta función controla el flujo del juego, permitiendo al jugador jugar varias veces si lo desea
function jugar() {
    while (true) {
        let jugarDeNuevo = confirm(queresJugar());
        if (jugarDeNuevo == true) {
            estableceDificultad();
            if (dificultad == null) {   // esta condicion analiza si el juego se cancelo antes de elegir una dificultad
                alert(adios());
                break;
            } else {
